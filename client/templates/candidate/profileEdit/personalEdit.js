Template.personalEdit.onRendered(function(){
            /*countries*/
    var countries = [ 
...
Template.personalEdit.onCreated(function() {
    var specialityInit = Meteor.user().profile.myProfile.speciality
    if (! specialityInit) {
        specialityArr = []
        Session.set('speciality', specialityArr)
    }
    else {
        Session.set('speciality', specialityInit)
    }
})

Template.personalEdit.onDestroyed(function() {
	updateCandidatePersonalInfo()
})

Template.personalEdit.helpers({
    speciality: function() {
        return Session.get('speciality');
    }
})

Template.personalEdit.events({
	'click .savePersonalInfo': function(event) {
		event.preventDefault();
		updateCandidatePersonalInfo()

	},
    'keyup .speciality': function(event) {
        event.preventDefault()
        var text = event.currentTarget.value
        $('.speciality').val(text.replace(/[^A-Za-z0-9-]/g,''));
    },
    'keydown .speciality': function(event) {
        if ((27 === event.which) || (13 === event.which)) {
            event.preventDefault();
            var data = event.target.value
            var specialityArr = Session.get('speciality')
            var len = specialityArr.length

            if (data && len <= 4) {
                specialityArr.push(data)
                Session.set('speciality', specialityArr)
    ...
    'click .removeSpeciality': function(event) {
        event.preventDefault();
        var specialityVal = this
        var specialityArray = Session.get('speciality')
        specialityArray = _.without(specialityArray, _.findWhere(specialityArray, specialityVal));
        Session.set('speciality', specialityArray)
    }
})
...
function updateCandidatePersonalInfo() {
    ...
    var specialityArr = Session.get('speciality')
  	  	...
  	  	Meteor.call('updateCandidatePersonal', locationObj, firstName, lastName, specialityArr,
